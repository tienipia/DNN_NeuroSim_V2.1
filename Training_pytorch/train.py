...
parser.add_argument('--epochs', type=int, default=257,
...
parser.add_argument('--decreasing_lr', default='200,250',
...
# cell precision (in V2.0, we only support one-cell-per-synapse, i.e. cellBit==wl_weight==wl_grad)
args.cellBit = 5
# Maximum number of conductance states during weight update (floor(log2(max_level))=cellBit)
args.max_level = 32
...
# nonlinearity in LTD (negative if LTP and LTD are asymmetric)
...
try:
    ...
    for epoch in range(args.epochs):
        ...
        for name, param in list(model.named_parameters()):
            ...
            k = k+1
            delta_std = np.append(
                delta_std, (torch.std(param.grad.data)).cpu().data.numpy())
            delta_mean = np.append(
                delta_mean, (torch.mean(param.grad.data)).cpu().data.numpy())
            w_std = np.append(
                w_std, (torch.std(param.data)).cpu().data.numpy())
            w_mean = np.append(
                w_mean, (torch.mean(param.data)).cpu().data.numpy())
        ...
        for i, layer in enumerate(model.features.modules()):
            if isinstance(layer, QConv2d) or isinstance(layer, QLinear):
                weight_file_name = './layer_record/weightOld' + \
                    str(layer.name) + '.csv'
                hook.write_matrix_weight(
                    (oldWeight[h]).cpu().data.numpy(), weight_file_name)
                h = h+1
        ...
        if epoch % args.test_interval == 0:
            ...
            np.savetxt(out, [[epoch, test_loss, accuracy]],
                       delimiter=",", fmt='%f')
